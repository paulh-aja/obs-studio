...
{
	if (check_buffer(repack, frame_count * 2) < 0)
		return -1;

	int squash = repack->extra_dst_size;
	const __m128i *src = (__m128i *)bsrc;
	const __m128i *esrc = src + (frame_count * 2);
		       ...
		       uint32_t frame_count)
{
	...
	while (src != esrc) {
		__m128i target = _mm_load_si128(src);
		src += 2;
		__m128i buf = _mm_shuffle_epi32(target, _MM_SHUFFLE(2, 3, 1, 0));
		      ...
		      audio_repack_mode_t repack_mode, uint8_t sample_bit)
{
	...
	repack->base_src_size = 8 * (32 / 8);
	repack->base_dst_size = _audio_repack_ch[repack_mode] * (32 / 8);
