...
void AJAOutput::Initialize(const OutputProps &props)
{
	...
	mOutputDelay = (int64_t)kFrameDelay * 1000000 * mFrameRateDen /
		      mFrameRateNum;
    mVideoMax = (int64_t)kVideoSyncAdjust * 1000000 * mFrameRateDen /
		      mFrameRateNum;
    mVideoMax -= 100;
	mAudioRate = props.audioSampleRate;
    mAudioMax = (int64_t)kAudioSyncAdjust * 1000000 / 48000;
...
//	blog(LOG_INFO, "time (v-a) %lld", ((int64_t)vf.frame.timestamp - (int64_t)mLastAudioTS)/1000000);
...
void AJAOutput::DMAVideoFromQueue()
{
    ...
    mVideoDelay = (((int64_t)mWriteCardFrame + (int64_t)mNumCardFrames -
                    (int64_t)mPlayCardFrame) % (int64_t)mNumCardFrames) *
	...
	while (ajaOutput->ThreadRunning()) {
		...
		if ((curTime - ajaOutput->mLastStatTime) > kDefaultStatPeriod) {
            ...
            if (ajaOutput->mAudioStarted)
            {
                ...
                if (ajaOutput->mAudioSync > ajaOutput->mAudioMax) {
                    ...
                    if (audioSyncSlowCount >= audioSyncCountMax) {
                        ajaOutput->mAudioAdjust =
                            audioSyncSlowSum / audioSyncCountMax;
                ...
                if (ajaOutput->mAudioSync < -ajaOutput->mAudioMax) {
                    ...
                    if (audioSyncFastCount >= audioSyncCountMax) {
                        ajaOutput->mAudioAdjust =
                            audioSyncFastSum / audioSyncCountMax;
            ...
            if (ajaOutput->mVideoDelay > 0) {
                ajaOutput->mVideoSync =
                    ajaOutput->mVideoDelay - ajaOutput->mOutputDelay;
			...
			blog(LOG_INFO,
                 ...
                 ajaOutput->mVideoDelay,
                 ajaOutput->mVideoSync,
                 ajaOutput->mVideoMax,
                 ajaOutput->mAudioDelay,
			     ajaOutput->mAudioSync,
                 ajaOutput->mAudioMax);
#endif
		}
