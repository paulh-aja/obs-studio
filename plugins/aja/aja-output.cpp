...
void AJAOutput::Initialize(const OutputProps &props)
{
    ...
    mVideoDelay =  ((int64_t)mNumCardFrames - 0) * 1000000 *
                mFrameRateDen / mFrameRateNum;
...
void AJAOutput::QueueAudioFrames(struct audio_data *frames, size_t size)
{
    ...
    mAudioQueueSamples += size / (kDefaultAudioChannels * kDefaultAudioSampleSize);
...
void AJAOutput::DMAAudioFromQueue(NTV2AudioSystem audioSys)
{
    ...
    if (mAudioPlayCursor <= mAudioWriteCursor) {
        audioPlaySamples = (mAudioWriteCursor - mAudioPlayCursor) /
            (kDefaultAudioChannels * kDefaultAudioSampleSize);
    }
    else {
        audioPlaySamples = (mAudioWrapAddress - mAudioPlayCursor + mAudioWriteCursor) /
            (kDefaultAudioChannels * kDefaultAudioSampleSize);
    }
    ...
    if (mAudioAdjust != 0) {
        if (mAudioAdjust > 0) {
            ...
            uint32_t adjustSamples = (uint32_t)mAudioAdjust * mAudioRate / 1000000;
            uint32_t adjustSize = adjustSamples * kDefaultAudioSampleSize * kDefaultAudioChannels;
            if (adjustSize <= sizeLeft) {
                af.offset += adjustSize;
                sizeLeft -= adjustSize;
                mAudioAdjust = 0;
                blog(LOG_DEBUG,
                     "AJAOutput::DMAAudioFromQueue: Drop %d audio samples", adjustSamples);
            } else {
                uint32_t samples = sizeLeft / (kDefaultAudioSampleSize * kDefaultAudioChannels);
                af.offset += sizeLeft;
                sizeLeft = 0;
                adjustSamples -= samples;
                mAudioAdjust = adjustSamples * 1000000 / mAudioRate;
                blog(LOG_DEBUG,
                     "AJAOutput::DMAAudioFromQueue: Drop %d audio samples", samples);
            }
        } else {
            ...
            mAudioAdjust = 0;
            blog(LOG_DEBUG,
                 "AJAOutput::DMAAudioFromQueue: Add %d audio samples", adjustSamples);
        }
    }
...
uint32_t AJAOutput::get_frame_count()
{
    ...
    bool isProgressiveTransport = NTV2_IS_PROGRESSIVE_STANDARD(::GetNTV2StandardFromVideoFormat(mOutputProps.videoFormat));
				  ...
				  size_t size)
{
    ...
    mAudioWriteSamples += size / (kDefaultAudioChannels * kDefaultAudioSampleSize);
...
void AJAOutput::OutputThread(AJAThread *thread, void *ctx)
{
	...
	while (ajaOutput->ThreadRunning()) {
        ...
        if (ajaOutput->mAudioStarted &&
            ((curTime - ajaOutput->mLastStatTime) > kDefaultStatPeriod))
        {
            ajaOutput->mLastStatTime = curTime;

            // Calculate av sync delay
            ajaOutput->mAudioVideoSync = ajaOutput->mAudioDelay - ajaOutput->mVideoDelay;

            if (ajaOutput->mAudioVideoSync > kAudioSyncAdjust) {
                audioSyncCount++;
                audioSyncSum += ajaOutput->mAudioVideoSync;
                if (audioSyncCount >= syncCountMax) {
                    ajaOutput->mAudioAdjust = audioSyncSum / syncCountMax;
                    audioSyncCount = 0;
                    audioSyncSum = 0;
                }
            } else {
            ...
            if (ajaOutput->mAudioVideoSync < -kAudioSyncAdjust) {
                ...
                if (videoSyncCount >= syncCountMax) {
                    ajaOutput->mAudioAdjust = videoSyncSum / syncCountMax;
                    videoSyncCount = 0;
                    videoSyncSum = 0;
                }
            } else {
            ...
            blog(LOG_DEBUG,
                 ...
                 ajaOutput->mVideoDelay, ajaOutput->mAudioDelay, ajaOutput->mAudioVideoSync);
#endif
        }
